...
function calcFare() {
  ...
  directionsService.route(request, function(response, status) {
    if (status == google.maps.DirectionsStatus.OK) {
      ...
      {
        if (distanceInput <= 60) {
          rates = distanceInput * rateObj[service][0] + rateObj[service][1] + extraCharges;
        } else if (distanceInput > 60) {
          rates =
            60 * 1.55 + (distanceInput - 60) * rateObj[service][3] + 3 + extraCharges;
        }
        rates = rates * 1.05;
        comm = rateObj[service] * rates;
      }
