...
function calcFare(vip) {
  ...
  directionsService.route(request, function(response, status) {
    if (status == google.maps.DirectionsStatus.OK) {
      ...
      const answer = toDollar(rates+extraCharges)
      ...
      if (!vip) {
        ...
        document.getElementById("grab").innerText = toDollar(calcGrabFare(distanceInput));
        calcFare(true);
      } else {
...
function calcGrabFare(dist) {
  // provide an estimate of Grab fare (in MYR) using distance (in km).
    return 1+ Math.round(dist*1.30)*2
}

function toDollar(myr) {
