...
function calcFare(vip, id) {
  ...
  directionsService.route(request, function(response, status) {
    if (status == google.maps.DirectionsStatus.OK) {
      ...
      const answer = toString(Math.round((rates + extraCharges + comm)*280)) + "Won";
      document.getElementById("taxi").innerText =
      // Total Price = rates + extraCharges + comm
    //   document.getElementById("taxi").innerText = Math.round(rates + extraCharges + comm);
    }
  });
}
