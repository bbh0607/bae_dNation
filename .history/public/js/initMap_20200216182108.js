...
function calcFare() {
  ...
  directionsService.route(request, function(response, status) {
    if (status == google.maps.DirectionsStatus.OK) {
      ...
      let budgetFare = calcTaxiFare(distanceInput, isAirport, false);
      let vipFare = calcTaxiFare(distanceInput, isAirport, true);
      ...
      document.getElementById("grab").innerText = myrToDollar(
        calcGrabFare(distanceInput)
        );
        calcFare(true);
      } else {
...
function calcTaxiFare(dist, isAirport, vip) {
  ...
  if (dist <= 60) {
    rates =
      dist * rateObj[service][0] +
      rateObj[service][1] +
      fromAirportCharge;
  } else {
    rates =
      60 * 1.55 +
      (dist - 60) * rateObj[service][3] +
      3 +
      fromAirportCharge;
  }
  rates = rates * 1.05;
