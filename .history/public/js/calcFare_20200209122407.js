...
function calcFare(vip) {
  ...
  directionsService.route(request, function(response, status) {
    if (status == google.maps.DirectionsStatus.OK) {
      ...
      const answer = Math.round(280*(rates+comm+extraCharges)).toString().concat(" Won");
      console.log(vip, document.getElementById("vip").value);
      if(!vip){
          document.getElementById("vip").innerText = answer} else{document.getElementById("taxi").innerText = answer);
    }
  });
}
