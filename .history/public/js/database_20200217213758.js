    ...
    .then(snapshot => {
      snapshot.forEach(doc => {
        ...
        let image =
          ...
          getRandomInt(2) +
          ".jpg";
        // let image = doc.data().scene_img;
        let coords = doc.data().location;
        let t = doc.data().time.split(":");
        let min = parseInt(doc.data().time.split(':')[0]);
        let sec = parseInt(doc.data().time.split(":")[1]);
        ...
        google.maps.event.addListener(marker, "click", function() {
          window.open(
            "http://www.youtube.com/watch?v=" + doc.data().video_id + "&t="+min+"m"+sec+"s", '_blank')
        });
      });
    })
    .catch(err => {
