...
function calcFare(vip) {
  ...
  directionsService.route(request, function(response, status) {
    if (status == google.maps.DirectionsStatus.OK) {
      ...
      let dur = response.routes[0].legs[0].duration.value;
    ...
    //   console.log(`total duration: ${dur} seconds`);
      ...
      const timeStr = sectoTime(dur)
      const answer = "$ ".concat(
        Math.round(0.24 * (rates + comm + extraCharges)).toString()
      );
      ...
      if (!vip) {
        document.getElementById("taxi").innerText = answer;
        document.getElementById("taxiTime").innerText =  
        calcFare(true);
      } else {
        document.getElementById("vip").innerText = answer;
      }
