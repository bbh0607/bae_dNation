...
function calcFare() {
  ...
  directionsService.route(request, function(response, status) {
    ...
    directionsService.route(publicRequest, function(response, status) {
      if (status == google.maps.DirectionsStatus.OK) {
        distanceInput = response.routes[0].legs[0].distance.value / 1000;
        const timeStr = sectoTime(response.routes[0].legs[0].duration.value);
        console.log("aaaaa", response.routes[0].fare);
        document.getElementById("busTime").innerText = timeStr;
        document.getElementById("bus").innerText = myrToDollar(
          response.routes[0].fare
        );
      }
    });
}
